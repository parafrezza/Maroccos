"""Scan and expose media files for upload operations."""

from __future__ import annotations

import threading
from dataclasses import dataclass
from pathlib import Path
from typing import Iterable

from GUI.core.logger import get_logger
from GUI.services.media_probe import probe_duration_seconds


_LOG = get_logger(__name__)


@dataclass(slots=True)
class MediaItem:
    """Describe a media file available for upload."""

    path: Path
    size: int
    duration_s: float | None = None


class MediaLibrary:
    """Index media files inside a root directory."""

    def __init__(self, root: Path) -> None:
        self._root = root

    def iter_items(self, *, cancel_event: threading.Event | None = None) -> Iterable[MediaItem]:
        """Yield all media files relative to the configured root."""
        if not self._root.exists():
            _LOG.warning("Media root %s does not exist", self._root)
            return
        for file_path in self._root.rglob("*"):
            if cancel_event and cancel_event.is_set():
                _LOG.debug("Media scan cancellata su richiesta")
                break
            if file_path.is_file():
                if cancel_event and cancel_event.is_set():
                    _LOG.debug("Media scan cancellata prima di elaborare %s", file_path)
                    break
                dur = probe_duration_seconds(file_path)
                yield MediaItem(path=file_path, size=file_path.stat().st_size, duration_s=dur)
