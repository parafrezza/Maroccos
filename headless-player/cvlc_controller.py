...
class CvlcController:
    ...
    def ensure_running(self) -> None:
        ...
        with self._proc_lock:
            ...
            _log.info("cvlc non è in esecuzione, avvio del processo...")
            env = os.environ.copy()
            ...
            try:
                ...
                _log.warning("Impossibile aprire il file di log per cvlc: %s", e)
                self._log_file = None

            def launch(cmd: list[str]) -> None:
                try:
                    _log.debug("Esecuzione comando: %s", " ".join(cmd))
                    ...
                    _log.error("Comando '%s' non trovato: %s", self.executable, exc)
                    raise VlcError(f"Impossibile eseguire '{self.executable}': {exc}") from exc
                except Exception as exc:  # pragma: no cover
                    _log.error("Errore imprevisto durante l'avvio di cvlc: %s", exc)
            ...
            try:
                _log.info("Tentativo di avvio di cvlc con interfaccia rc...")
                launch(cmd_full)
                self._wait_ready()
                _log.info("cvlc avviato correttamente con interfaccia rc.")
            except Exception as exc1:
                _log.warning("Avvio con interfaccia rc fallito: %s. Tentativo di fallback...", exc1)
                ...
                self._proc = None
                try:
                    _log.info("Tentativo di avvio di cvlc in modalità fallback (solo luahttp)...")
                    launch(cmd_fallback)
                    self._wait_ready()
                    _log.info("cvlc avviato correttamente in modalità fallback.")
                except Exception as exc2:
                    _log.error("Avvio di cvlc fallito anche in modalità fallback: %s", exc2)
    ...
    def _rc_send(self, line: str, timeout: float = 1.0) -> str:
        ...
        try:
            _log.debug("Invio comando RC: '%s'", line)
            with socket.create_connection(("127.0.0.1", 9090), timeout=timeout) as s:
                ...
                if any(bad in lt for bad in ("unknown", "not found", "invalid", "usage", "error")):
                    _log.warning("Comando RC '%s' rifiutato: %s", line, text.strip())
                    raise VlcError(f"RC rejected '{line}': {text.strip()}")
                return text
        except Exception as exc:
            _log.error("Fallimento invio comando RC '%s': %s", line, exc)
    ...
    def ensure_adjust_enabled(self) -> None:
        ...
        for cmd in (
            ...
            try:
                self._rc_send(cmd)
                _log.debug("Filtro 'adjust' abilitato con successo tramite: '%s'", cmd)
                return
            except VlcError:
                continue
        _log.warning("Nessun comando RC per abilitare 'adjust' ha avuto successo.")
    ...
    def set_brightness(self, value: float) -> None:
        ...
        for cmd in (
            ...
            try:
                ...
                last_err = str(exc)
        _log.error("Impossibile impostare la luminosità tramite RC: %s", last_err)
    ...
    def set_gamma(self, value: float) -> None:
        ...
        for cmd in (
            ...
            try:
                ...
                last_err = str(exc)
        _log.error("Impossibile impostare il gamma tramite RC: %s", last_err)
    ...
    def visual_fade_to(self, target: float, duration: float) -> None:
        ...
        _log.debug("Avvio visual fade a %.2f in %.2f secondi", target, duration)
            ...
            for i in range(steps + 1):
                with self._fade_lock:
                    if token != self._fade_token:
                        _log.debug("Visual fade interrotto da una nuova richiesta")
                        return
                fraction = i / steps
                ...
                try:
                    ...
                    _log.warning("Errore durante il visual fade, interruzione.")
                    return
    ...
    def shutdown(self, timeout: float = 5.0) -> None:
        with self._proc_lock:
            if not self._proc:
                return
            _log.info("Arresto del processo cvlc...")
            proc = self._proc
            self._proc = None
        ...
        try:
            ...
            _log.warning("Errore durante la terminazione di cvlc, forzatura in corso...")
            return
        try:
            proc.wait(timeout=timeout)
            _log.info("Processo cvlc arrestato correttamente.")
        except Exception:
            _log.warning("Timeout attesa arresto, forzatura kill di cvlc.")
            proc.kill()
        try:
            if logf:
    ...
    def restart(self) -> None:
        _log.info("Riavvio di cvlc richiesto.")
        self.shutdown()
        self.ensure_running()
    ...
    def _http_get(self, path: str, params: Optional[Dict[str, object]] = None) -> requests.Response:
        ...
        try:
            _log.debug("Richiesta HTTP GET a: %s con parametri: %s", url, params)
            resp = requests.get(url, params=params, auth=self._auth, timeout=2.0)
            resp.raise_for_status()
            return resp
        except requests.exceptions.RequestException as exc:
            _log.error("Richiesta HTTP a %s fallita: %s", url, exc)
    ...
    def _command(self, command: str, params: Optional[Dict[str, object]] = None) -> Dict[str, object]:
        ...
        try:
            ...
            _log.error("Risposta non JSON dal comando '%s': %s", command, exc)
    ...
    def status(self) -> Dict[str, object]:
        ...
        try:
            ...
            _log.error("Risposta status non JSON: %s", exc)
            raise VlcError(f"Risposta JSON non valida: {exc}") from exc
        if "volume" in data:
            try:
    ...
    def play(self, path: str, loop: bool = False, fade_in: float = 0.0) -> None:
        ...
        _log.info("Riproduzione di '%s' (loop: %s, fade-in: %.2fs)", path, loop, fade_in)
        self.ensure_running()
        ...
        if fade_in and fade_in > 0:
            ...
            _log.debug("Esecuzione fade-in del volume da 0 a %d in %.2fs", self.DEFAULT_VOLUME, fade_in)
            self.set_volume(0)
    ...
    def enqueue(self, path: str) -> None:
        media_uri = self._to_uri(path)
        _log.info("Aggiunta in coda di '%s'", path)
        self.ensure_running()
    ...
    def pause(self) -> None:
        ...
        if state != "paused":
            _log.debug("Messa in pausa della riproduzione")
    ...
    def resume(self) -> None:
        ...
        state = self.status().get("state")
        if state == "paused":
            _log.debug("Ripresa della riproduzione")
    ...
    def set_loop(self, enabled: bool) -> None:
        ...
        if current_loop != bool(enabled):
            _log.debug("Impostazione loop a: %s", enabled)
            self._command("pl_loop")

    def clear(self) -> None:
        _log.info("Pulizia della playlist")
        self.ensure_running()
        self._command("pl_empty")
    ...
    def prepare_media(self, path: str, wait_timeout: float = 2.0) -> None:
        ...
        """
        media_uri = self._to_uri(path)
        _log.info("Preparazione media '%s' per fast-start", path)
        self.ensure_running()
        # Svuota e riproduci
        try:
            self._command("pl_stop")
        ...
        try:
            ...
            _log.debug("Media '%s' preparato e in pausa al frame 0", path)
        except VlcError:
            pass
    ...
    def fade_to(self, target: int, duration: float) -> None:
        ...
        _log.debug("Avvio fade del volume a %d in %.2f secondi", target, duration)
        with self._fade_lock:
            self._fade_token += 1
            token = self._fade_token
        ...
        def worker() -> None:
            for i in range(steps + 1):
                with self._fade_lock:
                    if token != self._fade_token:
                        _log.debug("Fade del volume interrotto da una nuova richiesta")
                        return
                fraction = i / steps
                ...
                try:
                    ...
                    _log.warning("Errore durante il fade del volume, interruzione.")
                    return
                time.sleep(max(0.02, step_duration))
    ...
    def _wait_ready(self, timeout: float = 5.0) -> None:
        ...
        _log.debug("In attesa che cvlc diventi pronto su %s:%s...", self.host, self.port)
        while time.time() < deadline:
            try:
                resp = requests.get(
                ...
                if resp.status_code == 200:
                    _log.debug("cvlc è pronto.")
                    return
            except Exception as exc:  # pragma: no cover - dipende da runtime
                last_exc = exc
            time.sleep(0.2)
        _log.error("Timeout attesa cvlc. Ultimo errore: %s", last_exc)
    ...
    def reconfigure(
        ...
        if self.matches(host, port, user, password, extra_args):
            _log.debug("La configurazione di CvlcController non è cambiata.")
            return
        _log.info("Riconfigurazione di CvlcController con nuovi parametri. Riavvio in corso...")
        self.shutdown()
        self.host = host
        self.port = int(port)
        self.user = user
