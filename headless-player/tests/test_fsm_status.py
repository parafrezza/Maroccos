...
def test_next_ignored_with_single_item_playlist(fastapi_app):
	from fastapi.testclient import TestClient
	import importlib
	mod = importlib.import_module("app_module_fsm")
	media_dir = mod.MEDIA_DIR
	media_dir.mkdir(exist_ok=True)
	f = media_dir / "solo_next.mp4"
	_write_fake_mp4(f)
	with TestClient(fastapi_app) as client:
		resp = client.post("/playlist/apply", json={"items": ["solo_next.mp4"], "loop": True})
		assert resp.status_code == 200
		rn = client.post("/playlist/next")
		assert rn.status_code == 200
		body = rn.json()
		assert body.get("skipped") is True
		assert body.get("reason") == "single_item"
		status = client.get("/status")
		assert status.status_code == 200
		assert status.json().get("fsm", {}).get("action") in (None, "idle")


def test_prev_ignored_with_single_item_playlist(fastapi_app):
	from fastapi.testclient import TestClient
	import importlib
	mod = importlib.import_module("app_module_fsm")
	media_dir = mod.MEDIA_DIR
	media_dir.mkdir(exist_ok=True)
	f = media_dir / "solo_prev.mp4"
	_write_fake_mp4(f)
	with TestClient(fastapi_app) as client:
		resp = client.post("/playlist/apply", json={"items": ["solo_prev.mp4"], "loop": False})
		assert resp.status_code == 200
		rp = client.post("/playlist/prev")
		assert rp.status_code == 200
		body = rp.json()
