...
SUDOERS_FILE="/etc/sudoers.d/${APP_NAME}"
{
  ...
  echo "# Permesso per riavviare il proprio servizio da endpoint /system/service/restart"
  echo "${APP_USER} ALL=(ALL) NOPASSWD: /bin/systemctl restart ${APP_NAME}.service, /sbin/service ${APP_NAME} restart"
} > "$SUDOERS_FILE"
