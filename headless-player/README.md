...
- `POST /visual/ftb?seconds=...` → fade-to-black completo (audio + video) con stop automatico
...
## OFF backend: route più usate (GUI → headless → OFF)

Quando il backend attivo è "off", la GUI usa gli alias `/off/*` esposti dal server headless, che fanno passthrough verso l'istanza OFF-player.

### Configurazione eseguibile OFF-player

Il server headless rileva automaticamente l'eseguibile OFF-player nel percorso `../OFF-player/bin/` relativo alla directory del server:

- **Windows**: cerca `OFF-player.exe`
- **macOS**: cerca il bundle `OFF-player.app/Contents/MacOS/OFF-player`
- **Linux**: cerca `OFF-player` (senza estensione)

Per specificare un percorso custom, imposta la variabile d'ambiente `OFF_PLAYER_EXE`:
```bash
export OFF_BINARY=/path/to/custom/OFF-player
python app.py
```

**Nota**: Su macOS l'avvio automatico di OFF è abilitato di default (`OFF_AUTOSTART=true`). Su altre piattaforme, usa `POST /off/start` per avviare manualmente OFF-player.

- Process management:
  - `POST /off/start` → avvia OFF-player; il server sincronizza automaticamente la media dir su `headless-player/media/`
  - `GET|POST /off/process/stop` → termina il processo OFF-player
  - `GET /off/status` → stato processo OFF e (se raggiungibile) stato player remoto

- Playback (playlist OFF):
  - `GET|POST /off/play` → play della voce corrente
  - `GET|POST /off/stop` → stop playback (diverso da process stop!)
  - `GET|POST /off/pause` / `GET|POST /off/resume`
  - `GET|POST /off/next` / `GET|POST /off/prev`
  - `GET|POST /off/set?index=N` → seleziona indice in playlist OFF
  - `GET|POST /off/dir?path=/abs/media/dir` → imposta la cartella media per OFF (persistita nel suo `config.json`)
  - `GET|POST /off/reload` → ricostruisce la playlist in OFF
  - `GET|POST /off/loop?on=1|0`

- Visual e splash:
  - `POST /off/splash/show?text=...`, `POST /off/splash/hide`, `POST /off/splash/text?text=...`
  - `POST /off/visual/ftb?seconds=0.5` (fade to black), `POST /off/visual/fade_in?seconds=0.5`

Nota sulle differenze STOP:
- `POST /off/stop` → ferma solo il playback in OFF (l'app resta in esecuzione)
- `POST /off/process/stop` → termina il processo OFF-player

Play by filename (relativo a `media/`):
- `POST /play { filename: "clip.mp4" }` funziona anche con backend "off": il server risolve il percorso in `media/` e, quando possibile, mappa alla playlist OFF (allineando la dir e selezionando l'indice). In fallback, invia il path assoluto a `/play_file` di OFF.
...
- `POST /visual/ftb` ora combina fade audio e visivo, sfrutta overlay/backends dove disponibili e al termine invoca `/stop` automatico per garantire schermo nero stabile.
...
Sempre nero (idle):
- Quando lo splash è nascosto e non c'è playback attivo, `cvlc` genera automaticamente un PNG nero dedicato (salvato in `.idle/idle_black.png`), lo prepara e lo mette in pausa sul primo frame. Grazie a `VLC_EXTRA_ARGS --image-duration=36000`, l'immagine resta visibile senza scadere.
...
## Provisioning Windows (player dedicato)
Per ottimizzare un PC Windows 11 usato come player (risorse limitate, nessun uso generalista) è disponibile uno script di provisioning:

Cartella: `tools/windows/`
- `setup.bat` copia lo script di provisioning in `C:\\PlayerSetup` e può eseguirlo subito o al prossimo riavvio (RunOnce).
- `provision_player.ps1` applica ottimizzazioni (disabilitazione update, servizi superflui, visual minimal, IP statico su Ethernet, firewall rules, install TigerVNC, desktop nero).
- `rollback_player.ps1` ripristina elementi chiave (Windows Update, servizi core, DHCP su Ethernet, visual effetti).

Uso rapido (PowerShell amministratore):
```powershell
...
Parametri (principali) provisioning:
```
-NonInteractive          # nessun prompt
-EthernetAlias "Ethernet" # nome interfaccia per IP statico
-StaticIP 192.168.10.220 -PrefixLength 24
-DnsServers 8.8.8.8,1.1.1.1
-SkipTigerVNC            # salta installazione TigerVNC
```

Nota rete critica: l'IP statico uguale (es. 192.168.10.220) va usato SOLO se ogni player è isolato su una propria rete punto‑punto (es. cavo diretto verso un controller). Su una LAN condivisa genererebbe conflitti ARP e disconnessioni. In ambienti condivisi assegnare IP unici o usare DHCP.

TigerVNC: lo script tenta l'installazione via `winget`. Se non disponibile, apre la pagina ufficiale. Dopo l'installazione impostare la password VNC manualmente (in roadmap automazione `vncpasswd`).

Sicurezza: disattivare gli aggiornamenti di Windows riduce le modifiche impreviste ma espone a rischi di sicurezza se il device è su Internet. Isolare la macchina su reti chiuse o VLAN dedicata.
...
Extra:
- Se l’immagine nera non appare in idle con `cvlc`, verifica che `VLC_EXTRA_ARGS` includa `--image-duration=36000` (iniettato automaticamente dal servizio): l’asset viene rigenerato automaticamente nella cartella `.idle/idle_black.png` se necessario.
