...
# Log script origin so we can detect stale copies during provisioning
...
"@
        ...
        Write-Fail ("Impossibile impostare sfondo nero: {0}" -f $_.Exception.Message)
        throw
    } finally {
        if ($defaultLoaded) {
            $unloaded = $false
            ...
            if (-not $unloaded) {
                Write-Fail 'Impossibile scaricare hive Default: stato incoerente'
    ...
    if ($defaultUnloadFailure) {
        throw 'Scaricamento hive Default fallito'
    }
}
...
function Set-PowerButtonActionToShutdown {
    ...
    try {
        ...
        foreach ($scheme in @('SCHEME_CURRENT', 'SCHEME_MIN')) {
            powercfg /setacvalueindex $scheme $guidPowerButton $guidAction 1 | Out-Null
            powercfg /setdcvalueindex $scheme $guidPowerButton $guidAction 1 | Out-Null
        }
...
function Configure-EthernetStaticIP {
    ...
    try {
        ...
        Write-Fail ("Impossibile disattivare DHCP su {0}: {1}" -f $alias, $_.Exception.Message)
        throw
    }

    try {
        Get-NetIPAddress -InterfaceAlias $alias -AddressFamily IPv4 -ErrorAction SilentlyContinue | ForEach-Object {
            Remove-NetIPAddress -InterfaceAlias $alias -IPAddress $_.IPAddress -Confirm:$false -ErrorAction Stop
        }
    } catch {
        Write-Fail ("Impossibile rimuovere IP preesistenti da {0}: {1}" -f $alias, $_.Exception.Message)
        throw
    }
    ...
    try {
        ...
        Write-Fail ("Impossibile impostare IP statico {0} su {1}: {2}" -f $StaticIP, $alias, $_.Exception.Message)
        throw
    }
    ...
    if ($DnsServers -and $DnsServers.Length -gt 0) {
        try {
            ...
            Write-Fail ("Impossibile impostare DNS su {0}: {1}" -f $alias, $_.Exception.Message)
            throw
        }
    }
