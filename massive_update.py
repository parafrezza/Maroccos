...
def discover_players(ips: List[str], port: int, timeout: float, threads: int, api_key: Optional[str], verbose: bool = False):
    # Short-circuit per test: se NO_NETWORK_DISCOVERY abilitato evita thread e probing
    try:
        if os.environ.get("NO_NETWORK_DISCOVERY") in {"1", "true", "True"}:
            log("Discovery disabilitata (NO_NETWORK_DISCOVERY=1) - ritorno lista vuota")
            return []
    except Exception:
        pass
...
def _ensure_permissions(player: Dict, headers: Dict[str, str], timeout: float, verbose: bool = False) -> None:
    """Best-effort permission repair for Windows hosts before update apply."""
    status = player.get('status') or {}
    os_hint = (status.get('os') or '').lower()
    if not os_hint or ('win' not in os_hint and 'nt' not in os_hint):
        return

    ip = player.get('ip'); port = player.get('port')
    if not ip or port is None:
        return

    base = f"http://{ip}:{port}"

    def wait_status(deadline: float) -> tuple[Optional[str], Optional[dict]]:
        while time.time() < deadline:
            try:
                resp = requests.get(f"{base}/maintenance/status", headers=headers, timeout=min(timeout, 6.0))
                if resp.ok:
                    js = resp.json() or {}
                    state = js.get('status') or js.get('maintenance_status')
                    if state in {'ok', 'error'}:
                        return state, js
                elif verbose:
                    log(f"DEBUG PERM: {ip} status HTTP {resp.status_code}")
            except Exception as exc:
                if verbose:
                    log(f"DEBUG PERM: {ip} status errore: {exc}")
            time.sleep(1.0)
        return None, None

    # Se un altro processo sta già sistemando i permessi lasciamolo terminare
    state, info = wait_status(time.time() + 5.0)
    if state == 'ok':
        if verbose:
            log(f"DEBUG PERM: {ip} permessi già OK")
        return

    try:
        resp = requests.post(f"{base}/maintenance/fix_permissions", headers=headers, timeout=max(10.0, timeout * 1.5))
    except Exception as exc:
        log(f"[WARN] Permessi {ip}: errore avvio fix ({exc})")
        return

    if resp.status_code not in (200, 202, 409):
        log(f"[WARN] Permessi {ip}: fix_permissions HTTP {resp.status_code} -> {resp.text[:120]}")
        return
    if resp.status_code == 409 and verbose:
        log(f"DEBUG PERM: {ip} fix già in esecuzione")

    final_state, final_info = wait_status(time.time() + 180.0)
    if final_state == 'ok':
        log(f"[OK] Permessi {ip}: fix_permissions completato")
    elif final_state == 'error':
        msg = (final_info or {}).get('error')
        log(f"[FAIL] Permessi {ip}: fix_permissions errore: {msg}")
    else:
        log(f"[WARN] Permessi {ip}: fix_permissions timeout, stato sconosciuto")
